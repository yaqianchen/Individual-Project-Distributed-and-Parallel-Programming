...
int main (int argc,char *argv[]) {
    ...
    printf("Assignment 3, NBody Simulation, OpenMp Version\n");
    ...
    for(int k=0;k<iteration_num;k++){
        ...
        #pragma omp parallel num_threads(thread_num)
        {
            ...
            int recvCount = (thread_id == thread_num - 1) ? (body_num - chunk * (thread_num - 1)) : chunk;
            // cout << "the thread number is " << thread_num << endl;
            // cout << "the thread id is " << thread_id << ", the recvCount is " << recvCount << endl;
            for(int i=thread_id * chunk;i<thread_id * chunk + recvCount;i++)
            {
